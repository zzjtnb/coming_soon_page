...
function generateTime() {
  ...
  let dursec = "" + Math.floor(audio.value?.duration - Number(durmin) * 60);
  let curmin = "" + Math.floor(audio.value?.currentTime / 60);
  let cursec = "" + Math.floor(audio.value?.currentTime - Number(curmin) * 60);
  if (Number(durmin) < 10) {
    durmin = "0" + durmin;
  }
  if (Number(dursec) < 10) {
...
function updateBar(x) {
  let progre = progress.value;
  let maxduration = audio.value?.duration;
  let position = x - progre.offsetLeft;
  let percentage = (100 * position) / progre.offsetWidth;
  console.log(position, x, percentage);
  if (percentage > 100) {
    percentage = 100;
  }
  if (percentage < 0) {
  ...
  console.log(audio.value.currentTime, "ddd");
  audio.value?.play();
}
          ...
          <div class="player__top">
            <div class="player-cover">
              <transition-group :name="transitionName">
                <template v-for="(track, index) in tracks" :key="track.name"> <div class="player-cover__item" :style="{backgroundImage: `url(${track.cover})`}" v-if="index == currentTrackIndex"></div> </template>
              </transition-group>
            </div>
